"""Validate Grafana platform dashboard JSON (TASK-086).

Tests verify the dashboard JSON structure, panel definitions, and that
PromQL queries reference real metrics exposed by the platform (TASK-084).
"""
...
DASHBOARD_JSON = REPO_ROOT / "monitoring" / "grafana" / "dashboards" / "platform-overview.json"
HELM_DASHBOARD_JSON = (
    REPO_ROOT / "helm" / "ai-data-platform" / "dashboards" / "platform-overview.json"
)
...
class TestDashboardStructure:
    ...
    def test_title_is_set(self) -> None:
        dashboard = _load_dashboard(DASHBOARD_JSON)
        assert "AI Data Platform" in dashboard["title"]
...
class TestPanelDefinitions:
    ...
    def test_every_panel_has_type(self) -> None:
        dashboard = _load_dashboard(DASHBOARD_JSON)
        for panel in dashboard["panels"]:
...
class TestPromQLReferencesRealMetrics:
    ...
    def test_dashboard_covers_errors(self) -> None:
        dashboard = _load_dashboard(DASHBOARD_JSON)
        titles = [p["title"].lower() for p in dashboard["panels"]]
        assert any("error" in t for t in titles)

    def test_dashboard_covers_source_health(self) -> None:
        dashboard = _load_dashboard(DASHBOARD_JSON)
        titles = [p["title"].lower() for p in dashboard["panels"]]
        assert any("source" in t or "freshness" in t for t in titles)

    def test_dashboard_covers_api(self) -> None:
        dashboard = _load_dashboard(DASHBOARD_JSON)
        titles = [p["title"].lower() for p in dashboard["panels"]]
