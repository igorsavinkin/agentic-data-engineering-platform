...
and the local Codex CLI supporting `exec --approve-for-me`.
...
Codex runs with `--approve-for-me`, which selects the workspace-write sandbox and routes
...
Do not combine it with `--sandbox`; the CLI treats these options as mutually exclusive.
