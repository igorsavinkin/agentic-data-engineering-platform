...
description: >
  ...
  alibaba/open-code-review. Use when the user asks to review code, review a
  pull request, review staged/unstaged changes, review a commit, or compare
  branches for code quality issues. Produces line-level review comments and
...
```json
{
  "rules": [
    {
      "path": "**/*.py",
      "rule": "All new functions must include type hints for parameters and return values",
      "merge_system_rule": true
    },
    {
      "path": "**/*.ts",
      "rule": "Avoid using any type; use unknown or proper generics instead"
    }
  ]
}
```
...
```bash
ocr rules check src/main/python/example.py
```
