"""Silver Parquet writer — persist validated events to MinIO/S3 (TASK-022).

This module owns the boundary between validated ``ProductObservationEvent``
objects and Silver-layer Parquet files in object storage.  It provides:
...
ONLY after ``flush_batch`` or ``write_event`` succeeds.  If the write fails,
the offset is NOT committed and the same record will be redelivered on
restart/rebalance.  Because object keys are deterministic (derived from
``event_id``), replay is safe even if a prior attempt partially succeeded.

Partitioning strategy
---------------------
Silver data is partitioned by source and time, mirroring Bronze:
...
still enabling efficient time-range queries per source.  The parallel layout
between Bronze and Silver simplifies cross-layer joins and reconciliation.
"""
...
def build_silver_partition_key(event: ProductObservationEvent) -> str:
    """Return the S3/MinIO key for a single-event Silver file.

    The key encodes source and temporal dimensions so that listing by prefix
    yields all validated observations for a given source/date range.  The leaf
    name is the ``event_id`` to guarantee idempotency across replays.
    """
...
def validated_event_to_row(event: ProductObservationEvent) -> dict[str, Any]:
...
@dataclass
class SilverBatch:
    """Accumulate validated events until a flush threshold is reached."""
...
class SilverWriter:
    """Write validated events to Silver Parquet in MinIO/S3.
    ...
    bucket:
        The Silver bucket name (defaults to settings value).
    batch_size:
    ...
    def __init__(
        ...
        self._bucket = bucket
        self._batch = SilverBatch(events=[], max_size=batch_size)
        logger.info(
    ...
    def write_event(self, event: ProductObservationEvent) -> None:
        """Persist a single validated event to Silver Parquet immediately.
    ...
    def flush_batch(self) -> None:
        ...
        if errors:
            raise StorageError(
                f"silver_flush failed for {len(errors)} event(s): "
