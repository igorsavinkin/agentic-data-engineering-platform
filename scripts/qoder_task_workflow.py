"""One-task Qoder/Qwen workflow using Qoder chat sessions. See docs/AUTOMATED_TASK_WORKFLOW.md."""
...
class Workflow:
    ...
    def implement(self, feedback: str = "") -> None:
        ...
        # Create a Qoder chat session targeting the worktree directory
        session_prompt = f"Working directory: {self.worktree}\n\n{prompt}"

        # Use create_chat_session to spawn an independent Qoder conversation
        # The session runs in the worktree environment with full tool access
        session_result = self._create_qoder_session(session_prompt)
        session_id = session_result["sessionId"]

        # Wait for the session to complete (idle or requires human input)
        self._wait_for_session(session_id)

        # Read the session transcript for logging
        transcript = self._read_session(session_id)
        (self.directory / f"qoder-{attempt}.txt").write_text(transcript, encoding="utf-8")

        clean(self.worktree)
        head = self.identity()
        if head == before:
            raise WorkflowError("Qoder made no commit; inspect its output before resuming")
        self.save("review")

    def _create_qoder_session(self, prompt: str) -> dict[str, Any]:
        """Create a Qoder chat session in the worktree environment."""
        # Import here to avoid requiring qoder tools outside this workflow
        try:
            from qoder_tools import create_chat_session  # type: ignore
        except ImportError:
            raise WorkflowError(
    ...
    def _wait_for_session(self, session_id: str, timeout: int = 3600) -> None:
        ...
        while time.monotonic() < deadline:
            result = wait_chat_sessions(sessionIds=[session_id], timeoutMs=30000)
            # Check if session is idle or completed
            if result and any(s.get("status") in {"idle", "completed", "failed"} for s in result):
                return
            time.sleep(5)
        raise WorkflowError(f"Qoder session timed out after {timeout}s")
    ...
    def _read_session(self, session_id: str) -> str:
        ...
        result = read_chat_session(sessionId=session_id, turnLimit=50)
        # Extract the last assistant message as the transcript summary
        messages = result.get("turns", [])
        if not messages:
            return ""

        # Build a simple transcript from the turns
        lines = []
